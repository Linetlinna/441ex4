...
function renderCart709() {  
    ...
    document.getElementById('total').textContent = `Total: AUD ${totalPrice.toFixed(2)}`; // 使用 toFixed 保留两位小数  
}  
  
function addItem709(itemName, itemPrice) {  
    if (itemName && itemPrice) {  
        cart.push({ name: itemName, price: parseFloat(itemPrice) }); // 确保 itemPrice 是数字  
        renderCart709();  
    }   
}  
